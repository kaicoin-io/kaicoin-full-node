...
#ifndef KAICOINPARAMS_H
#define	KAICOINPARAMS_H
...
typedef struct mc_OneKaicoinParam
{    
...
typedef struct mc_KaicoinParams
{    
    char *m_lpData;
    mc_MapStringIndex *m_lpIndex;
    mc_OneKaicoinParam *m_lpParams;
    int *m_lpCoord;
    int m_Status;
    int m_Size;
    int m_Count;
    int m_IsProtocolKaiCoin;
    int m_ProtocolVersion;
    ...
    int Clone(const char *name,mc_KaicoinParams *source);
    ...
    const mc_OneKaicoinParam *FindParam(const char *param);
    ...
    int IsParamUpgradeValueInRange(const mc_OneKaicoinParam *param,int version,int64_t value);
