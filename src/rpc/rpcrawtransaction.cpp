...
Value appendrawchange(const Array& params, bool fHelp)
{
    ...
    {
        if(ParseKaicoinTxOutToBuffer(0,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
    ...
    {
        if(ParseKaicoinTxOutToBuffer(tx.vin[i].prevout.hash,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
            nAmount+=txout.nValue;
        }
        i++;
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=kaicoin");
    }    
