...
int mc_State::VersionInfo(int version)
{
    ...
    if(version < KAICOIN_VERSION_CODE_MIN_VALID)
    {
        switch(version)
        {
            case KAICOIN_VERSION_CODE_BUILD:                                 // last version
                return -this_build;                                               
            case KAICOIN_VERSION_CODE_PROTOCOL_THIS:                         // last protocol version
                return this_protocol;
            case KAICOIN_VERSION_CODE_PROTOCOL_MIN:
                return 10004;                                                   // first supported version
            case KAICOIN_VERSION_CODE_PROTOCOL_MIN_DOWNGRADE:
                return 10008;                                                   // cannot downgrade below this version
            case KAICOIN_VERSION_CODE_PROTOCOL_MIN_NO_DOWNGRADE:
                return 20002;                                                   // if we are on this version or above, downgrades are forbidden
            case KAICOIN_VERSION_CODE_PROTOCOL_FOR_RELEVANCE:                
    ...
    return VersionInfo(KAICOIN_VERSION_CODE_BUILD)-1;                        // Created by the following builds
}
...
int mc_State::IsDeprecated(int version)
{
    ...
    if(ptr)
    {
        if(strcmp((char*)ptr,"multichain"))
        {
            is_protocol_kaicoin=0;
        }
    }
    
    if(is_protocol_kaicoin)
    {
        if(build > 0)
        {
...
{
    return -VersionInfo(KAICOIN_VERSION_CODE_BUILD);
}

int mc_State::GetProtocolVersion()
{
    return VersionInfo(KAICOIN_VERSION_CODE_PROTOCOL_THIS);
}

int mc_State::MinProtocolVersion()
{
    return VersionInfo(KAICOIN_VERSION_CODE_PROTOCOL_MIN);
}

int mc_State::MinProtocolDowngradeVersion()
{
    return VersionInfo(KAICOIN_VERSION_CODE_PROTOCOL_MIN_DOWNGRADE);
}

int mc_State::MinProtocolForbiddenDowngradeVersion()
{
    return VersionInfo(KAICOIN_VERSION_CODE_PROTOCOL_MIN_NO_DOWNGRADE);    
}

int mc_State::RelevantParamProtocolVersion()
{
    return VersionInfo(KAICOIN_VERSION_CODE_PROTOCOL_FOR_RELEVANCE);        
}
