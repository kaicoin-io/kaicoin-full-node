...
bool KaicoinNode_DisconnectRemote(CNode *pnode)
{
    ...
    {
        return !KaicoinNode_CanConnect(pnode);
    }
...
bool KaicoinNode_AcceptData(CNode *pnode)
{
    ...
    return !KaicoinNode_DisconnectRemote(pnode);
}
...
bool VerifyKaicoinVerackHash(string sParameterSetHash,uint64_t nNonce)
{
...
{   
    ...
    {
        if(!VerifyKaicoinVerackHash(sParameterSetHash,nNonce))
        {
            if(!fIsVerackack)
            {
...
//                return false;
    ...
    if(MCP_ANYONE_CAN_CONNECT == 0)
    {
        ...
        if(fIsVerackack)
        {
            ...
            if(!KaicoinNode_CanConnect(pfrom))
            {
