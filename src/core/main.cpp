...
bool AcceptKaiCoinTransaction(const CTransaction& tx, 
...
bool ProcessKaicoinVerack(CNode* pfrom, CDataStream& vRecv,bool fIsVerackack,bool *disconnect_flag);
bool PushKaiCoinVerack(CNode* pfrom, bool fIsVerackack);
bool KaicoinNode_CanConnect(CNode *pnode);
bool KaicoinNode_DisconnectRemote(CNode *pnode);
bool KaicoinNode_DisconnectLocal(CNode *pnode);
bool KaicoinNode_RespondToGetData(CNode *pnode);
bool KaicoinNode_SendInv(CNode *pnode);
bool KaicoinNode_AcceptData(CNode *pnode);
bool KaicoinNode_IgnoreIncoming(CNode *pnode);
bool KaicoinNode_IsLocal(CNode *pnode);
...
const string strMessageMagic = "KaiCoin Signed Message:\n";
...
int SetUpgradedParamValue(const mc_OneKaicoinParam *param,int64_t value)
{
    ...
    if(strcmp(param->m_Name,"targetblocktime") == 0)
    {
        ...
        SetKaiCoinParam("targetblocktime",value);
    }   
...
int KaicoinNode_ApplyUpgrades(int current_height)
{
...
int KaicoinNode_ApplyUpgrades_Old(int current_height)
{
...
void KaicoinNode_UpdateBlockByHeightList(CBlockIndex *pindex)
{
    if(pindex->nHeight < 0)
    {
        return;
...
bool KaicoinNode_IsBlockChainSynced(CNode *pnode)
{
...
/* MCHN START */
        ...
        if(mc_gState->m_Features->Streams())
        {
            if(!AcceptKaiCoinTransaction(tx,view,-1,true,reason, &replay))
            {
                return state.DoS(0,
                                 error("AcceptToMemoryPool: : AcceptKaiCoinTransaction failed %s : %s", hash.ToString(),reason),
                                 REJECT_NONSTANDARD, reason);
            }
        }
        else
...
/* MCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        ...
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if(mc_gState->m_Features->Streams())
            {
                if(!AcceptKaiCoinTransaction(tx,view,offset,true,reason,NULL))
                {
...
/* MCHN START */        
            ...
            if(!fJustCheck)
            {
                if(mc_gState->m_Features->Streams())
                {
                    ...
                    {
                        return state.DoS(0,
                                         error("ConnectBlock: : AcceptKaiCoinTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                         REJECT_NONSTANDARD, reason);
                    }
                }
                else
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(mc_gState->m_Features->Streams())
                {
                    if(!AcceptKaiCoinTransaction(tx,view,coinbase_offset,true,reason,NULL))
                    {
                        return false;       
                    }
                }
...
/* MCHN START */    
    ...
    KaicoinNode_ApplyUpgrades(old_height-1);        
    if(mc_gState->m_WalletMode & MC_WMD_TXS)
    {
...
/* MCHN START */        
    ...
    KaicoinNode_ApplyUpgrades(chainActive.Height());    
/* MCHN END */    
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    KaicoinNode_UpdateBlockByHeightList(pindexNew);
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex, int node_id, CBlockIndex *pindexChecked)
{
    ...
    if( (node_id > 0) && (pindexPrev != NULL) )
    {
        ...
        {
            KaicoinNode_UpdateBlockByHeightList(pindexPrev);
        }
        lpNextForPrev=NULL;
...
/* MCHN START */    
    if(!KaicoinNode_RespondToGetData(pfrom)) 
    {
...
/* MCHN START */
        if(mc_gState->m_NetworkParams->IsProtocolKaicoin())
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(!ProcessKaicoinVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                    ...
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushKaiCoinVerack(pfrom,true);
                        }
                        else
                        {
    ...
    else if (strCommand == "verackack")
    {
        if(mc_gState->m_NetworkParams->IsProtocolKaicoin())
        {
            ...
            if(!ProcessKaicoinVerack(pfrom,vRecv,true,&disconnect_flag))
            {
            ...
            {
                pfrom->fDisconnect |= !PushKaiCoinVerack(pfrom,false);       // MCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                mc_gState->m_NetworkState = MC_NTS_SEED_READY;
...
/* MCHN END */
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!KaicoinNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !KaicoinNode_IgnoreIncoming(pfrom))      // MCHN
    ...
    else if (strCommand == "tx" && KaicoinNode_AcceptData(pfrom))            // MCHN
    {
/* MCHN START */        
        if(!KaicoinNode_IgnoreIncoming(pfrom))
        {
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // MCHN
    else if (strCommand == "block" && !fImporting && !fReindex && KaicoinNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* MCHN START */        
...
/* MCHN END */        
    ...
    else if (strCommand == "mempool" && KaicoinNode_SendInv(pfrom))          // MCHN
    {
...
/* MCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(KaicoinNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
            ...
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && KaicoinNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
...
/* MCHN END */            
        ...
        {
/* MCHN START */            
            if(pfrom->fDisconnect || !KaicoinNode_DisconnectRemote(pfrom))
            {
/* MCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(KaicoinNode_SendInv(pto))                         // MCNN
...
/* MCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=KaicoinNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
/* MCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && KaicoinNode_IsBlockChainSynced(pto))
            {
