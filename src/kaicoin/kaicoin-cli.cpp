...
std::string HelpMessageCli()
{
    ...
    strUsage += "  -cold                    " + _("Connect to kaicoind-cold: use kaicoind-cold default directory if -datadir is not set") + "\n";
/* MCHN START */    
    strUsage += "  -requestout=<requestout> " + _("Send request to stderr, stdout or null (not print it at all), default stderr") + "\n"; 
    strUsage += "  -saveclilog=<n>          " + _("If <n>=0 kaicoin-cli history is not saved, default 1") + "\n";
/*    
...
//    ParseParameters(argc, argv);
        ...
        mc_gState->m_Params->m_NumArguments<minargs)
      {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  kaicoin-cli <blockchain-name> [options] <command> [params]  " + _("Send command to KaiCoin Core") + "\n" +
                  "  kaicoin-cli <blockchain-name> [options] help                " + _("List commands") + "\n" +
                  "  kaicoin-cli <blockchain-name> [options] help <command>      " + _("Get help for a command") + "\n";
...
*/
    ...
    SelectKaiCoinBaseParams(mc_gState->m_Params->NetworkName(),RPCPort);
...
// Setting Argv for Kaicoin
        argc++;
        char *temp1 = (char*)malloc(sizeof(char)*255);
        char *temp2 = (char*)malloc(sizeof(char)*255);
        strcpy(temp2, argv[1]);
        strcpy(temp1, "kaicoin");
        argv[1] = temp1;
        argv[2] = temp2;
//
    ...
    if(mc_gState->m_Params->m_NumArguments == 1)                                // Interactive mode
    {
        ...
        while(!exitnow && ((command=term->GetLine()) != NULL))
        {
            ...
            else
            {
                ...
                offset=0;
                argc_p=0;
                strcpy(dest+offset,"kaicoin-cli");
