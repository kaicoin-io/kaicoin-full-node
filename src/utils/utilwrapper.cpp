...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\KaiCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\KaiCoin
    // Mac and Unix: ~/.kaicoin
#ifdef WIN32
    // Windows
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return GetSpecialFolderPath(CSIDL_APPDATA) / "KaiCoinCold";
    }
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KaiCoin";
#else
    // Mac and Unix
    ...
    if(mc_gState->m_SessionFlags & MC_SSF_COLD)
    {
        return pathRet / ".kaicoin-cold";        
    }
    return pathRet / ".kaicoin";
#endif
}
...
int mc_KaicoinParams::SetProtocolGlobals()
{
...
int mc_KaicoinParams::SetGlobals()
{
...
int mc_KaicoinParams::Import(const char *name,const char *source_address)
{
    
    return MC_ERR_NOERROR;
}

std::string KaicoinServerAddress()
{
