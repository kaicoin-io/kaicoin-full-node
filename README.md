...
Compile KaiCoin for Ubuntu (64-bit)
-----------------------------
...
* The release is built with GCC after which `strip kaicoind` strings the debug symbols, which reduces the executable size by about 90%.
...
To create a statically linked KaiCoin which only depends on default MacOS dylibs, the following steps are taken:
